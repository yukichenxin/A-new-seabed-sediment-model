...
from utils.utils import (cvtColor, get_classes, preprocess_input, resize_image,resize_image1,
                         show_config)
...
class YOLO(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.num_classes+1 <= 21:
            ...
            hsv_tuples = [(x / self.num_classes+1, 1., 1.) for x in range(self.num_classes+1)]
            self.colors_s = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
            self.colors_s = list(map(lambda x: (int(x[0] * 255), int(x[1] * 255), int(x[2] * 255)), self.colors_s))
        #---------------------------------------------------#
        #   获得种类和先验框的数量
        #---------------------------------------------------#
        # self.class_names, self.num_classes  = get_classes(self.classes_path)
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, crop = False, count = False,name_classes=None):
        #---------------------------------------------------#
        #   获得输入图片的高和宽
        #---------------------------------------------------#
        ...
        _, nw, nh = resize_image1(image, (self.input_shape[1], self.input_shape[0]))
        #---------------------------------------------------------#
        #   添加上batch_size维度
        #---------------------------------------------------------#
        ...
        if count:
            ...
            classes_nums = np.zeros([self.num_classes+1])
